...
func DefaultRiskPolicies(now time.Time) []RiskPolicy {
	definitions := []RiskPolicy{
		...
		{ID: "pol_unbatched_dml", Code: "UNBATCHED_LARGE_DML", Name: "大批量 DML 缺少分批边界", Description: "无 LIMIT/分批上限的 UPDATE/DELETE 容易形成大事务，放大锁持有时间和 WAL/回滚成本。", Severity: RiskMedium, Enabled: true, Builtin: true, Suggestion: "按主键或时间窗口分批，单批限制行数并控制提交节奏；生产执行前在影子库验证锁等待。"},
		{ID: "pol_fk_not_valid", Code: "FK_WITHOUT_NOT_VALID", Name: "外键约束缺少 NOT VALID 分阶段", Description: "直接 ADD FOREIGN KEY 会在创建时扫描整表校验，长事务期间持有 SHARE ROW EXCLUSIVE 等锁。", Severity: RiskHigh, Blocking: true, Enabled: true, Builtin: true, Suggestion: "先 ADD CONSTRAINT ... NOT VALID，低峰期再 VALIDATE CONSTRAINT；或拆成独立维护窗口。"},
		{ID: "pol_select_for_update", Code: "SELECT_FOR_UPDATE_UNBOUNDED", Name: "无边界行锁查询", Description: "SELECT ... FOR UPDATE 缺少 LIMIT/明确主键范围时，可能锁住过量行并拉长事务。", Severity: RiskMedium, Enabled: true, Builtin: true, Suggestion: "缩小锁定范围，补充 LIMIT 或主键条件，并缩短事务内业务逻辑。"},
		{ID: "pol_heavy_ddl", Code: "HEAVY_DDL_REWRITE", Name: "重写型 DDL 长事务风险", Description: "VACUUM FULL、CLUSTER、REINDEX、ALTER TYPE 等可能触发表重写与长事务，阻塞读写。", Severity: RiskHigh, Blocking: true, Enabled: true, Builtin: true, Suggestion: "改用 CONCURRENTLY/在线工具，或拆分维护窗口；设置 lock_timeout 并准备可观测回滚。"},
		{ID: "pol_missing_lock_timeout", Code: "MISSING_LOCK_TIMEOUT", Name: "高锁风险变更未声明锁超时", Description: "可能持锁的 DDL/大批量 DML 未声明 lock_timeout，锁等待会无限堆积并放大雪崩。", Severity: RiskMedium, Enabled: true, Builtin: true, Suggestion: "在执行脚本中设置合理 lock_timeout/statement_timeout，冲突时快速失败并重试。"},
		{ID: "pol_mixed_ddl_dml", Code: "MIXED_DDL_DML_TRANSACTION", Name: "同一变更混用 DDL 与大批量 DML", Description: "DDL 与大批量 DML 同单执行会拉长事务边界，失败回滚和锁竞争更难控制。", Severity: RiskMedium, Enabled: true, Builtin: true, Suggestion: "将结构变更与数据回填拆成独立变更单，分别演练与审批。"},
