...
| 通行证重放或并发消费 | 明文 Token 只返回一次，服务端只存 SHA-256。文件模式在写锁下、PostgreSQL 模式通过条件 UPDATE 将 `ACTIVE` 改为 `CONSUMED`，并将变更原子标为 `COMPLETED`。 | `COMPLETED` 只表示 Gate 已消费。消费后部署失败需要重新提交和审批，不能重放旧 Token。 | `TestUsePassportConcurrentConsumeCompletesChangeOnce` |
...
| 持久化失败造成半更新 | 文件存储保存失败时恢复内存快照；PostgreSQL 核心更新和审计走事务路径。 | 服务可能在状态落盘后、响应返回前崩溃。客户端应查询状态后再决定下一步。 | `TestUsePassportSaveFailureRollsBackPassportAndChange`；PostgreSQL 契约测试 |
