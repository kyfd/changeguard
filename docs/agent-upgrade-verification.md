...
| --- | --- |
| C1 重试分层 | ✅ 完成 |
| C2 模型不得自封 `confirmed` | ✅ 完成 |
| C3 版本与修订说明 | ✅ 完成 |
| C6 方言支持范围 | ✅ 完成 |
| C4 `ClarifyRequest.note` / C5 `schema_snapshot` | ✅ 完成 |
| **受约束调查循环** | ⬜ **未开始**——P1 主体 |
| 预算（最大轮次、累计工具调用、单工具超时、任务总时限、输出预算）与空转检测 | ⬜ 未开始 |
| 模型自主工具选择（决策契约：`call_tool` / `request_clarification` / `finish` / `stop`） | ⬜ 未开始 |
| 必需证据的确定性完成条件 | ⬜ 未开始 |
| 工具结果的结构化截断与证据标识/版本/摘要哈希 | ⬜ 未开始 |
| usage 缺失时的预算策略（不得把未知 token/费用填 0） | ⬜ 未开始 |
| `fixed_workflow` / `bounded_agent` 策略开关 | ⬜ 未开始 |

**不得把本节读成"P1 已完成"。** 上述前置修复之所以先做，是因为调查循环会建立在它们之上；
循环本身尚未实现，也尚无任何脚本化 planner 的覆盖。
